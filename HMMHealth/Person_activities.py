...
def createSample(n,l,k):
    ...
    iden=np.identity(k)
    probstate=np.random.rand(k)*0.4
    pi = probstate * iden
    #pi=np.identity(k) * 0.6
    trans=np.zeros([k,k])
    ...
    transmat_temp = pi + trans
    transmat_org = transmat_temp / np.sum(transmat_temp, 1).reshape((k, 1))

    #print transmat_org

    #print np.sum(transmat_org, 1).reshape((k, 1))

    #/ (1 + probstate).reshape((k, 1))
    #print transmat_org
    ...
    for j in range(0,l):
        for i in range(1,k):
            means_org[i,j]=means_org[i-1,j]+100
...
if __name__=="__main__":
    ...
    input=genfromtxt('experiment_values_1.csv', delimiter=',',dtype='int')
    input=input[1:,1:]
    ...
    for i in range(4,35):
        ...
        outputarr[i-4,:]= exp,timeTrain.total_seconds(),timeTest.total_seconds(),n,k,l,acctrain,acctest,min_start,min_transmat,min_means,min_covars

        #print exp,timeTrain.total_seconds(),timeTest.total_seconds(),n,k,l,acctrain,acctest,min_start,min_transmat,min_means,min_covars


    #print outputarr

    np.savetxt('experiment2_.out',outputarr, delimiter=',')
